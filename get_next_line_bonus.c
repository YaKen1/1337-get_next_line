...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/01 23:05:02 by ykenzaou          #+#    #+#             */
/*   Updated: 2023/04/01 23:05:06 by ykenzaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buf)
		return (0);
	next_line = read_and_join_lines(fd, buf, rest[fd]);
	free(buf);
	buf = NULL;
	if (!next_line)
		return (NULL);
	rest[fd] = extract_next_line(next_line);
	return (next_line);
}
